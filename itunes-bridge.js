...
if (process.platform === "darwin") {
  ...
  var libPath = path.resolve(
    os.homedir() + "/My Music/iTunes/iTunes Library.xml"
  );
}

if (config.get("performanceMode"))
  console.log("Started iTunes-bridge in Performance-Mode");
...
setInterval(function () {
  ...
  if (currentTrack && that.currentTrack) {
    if (
      ...
      event.emit("timeChange", "track", currentTrack);
    // On track change
    /**
     ...
     */
    if (
      currentTrack.id !== that.currentTrack.id &&
      currentTrack.playerState === "playing"
    ) {
     ...
     */
      currentTrack.id !== that.currentTrack.id &&
      currentTrack.playerState === "paused"
    ) {
      that.currentTrack = currentTrack;
      /**
       * Paused event
    ...
    if (
      currentTrack.playerState !== that.currentTrack.playerState &&
      currentTrack.id === that.currentTrack.id
    ) {
...
function runScript(req, type, args, isJson) {
  if (process.platform === "darwin") {
    var iTunesCtrlScpt = path
      .join(__dirname, "/jxa/iTunesControl.js")
      .replace("app.asar", "app.asar.unpacked");
    var iTunesFetcherScpt = path
      .join(__dirname, "/jxa/iTunesFetcher.js")
      .replace("app.asar", "app.asar.unpacked");
    switch (type) {
      case "fetch": {
        if (isJson) {
    ...
    var iTunesCtrlScpt = path
      .join(__dirname, "/wscript/iTunesControl.js")
      .replace("app.asar", "app.asar.unpacked");
    var iTunesFetcherScpt = path
      .join(__dirname, "/wscript/iTunesFetcher.js")
      .replace("app.asar", "app.asar.unpacked");
    switch (type) {
      case "fetch": {
        if (isJson) {
